...
func (store *SuggestionJobStore) complete(jobID string, result []DocumentSuggestion) {
	store.Lock()
	defer store.Unlock()
	if job, exists := store.jobs[jobID]; exists {
...
func processSuggestionJob(app *App, job *SuggestionJob) {
	...
	result, err := app.generateDocumentSuggestionsSequential(jobCtx, job.Request, job.ID, suggestionLogger.WithField("job_id", job.ID))
	if err != nil {
		switch jobCtx.Err() {
	...
	suggestionJobStore.complete(job.ID, result)
	suggestionLogger.Infof("Suggestion job completed: %s", job.ID)
}
