...
func TestOllamaMetadataSettingsMergeAndExplicitZeros(t *testing.T) {
	...
	model, err := newOllamaMetadataModel(server.URL, "model", 4096, boolPointer(true), 0.7, nil, OllamaMetadataConfig{
		...
		Prompts: map[string]OllamaGenerationSettings{
			"title.prompt": {Temperature: floatPointer(0.2), TopP: floatPointer(0.4), Stop: &promptStop},
		},
	})
	require.NoError(t, err)

	_, err = model.Call(context.Background(), "global")
	require.NoError(t, err)
	_, err = model.Call(withOllamaPrompt(context.Background(), "title.prompt"), "prompt")
	require.NoError(t, err)
	_, err = model.Call(withOllamaPrompt(context.Background(), "title.prompt"), "override", llms.WithTemperature(0), llms.WithTopP(0), llms.WithSeed(0), llms.WithStopWords([]string{}), llms.WithMaxTokens(0))
	require.NoError(t, err)
	...
	assert.Equal(t, map[string]any{"temperature": 0.2, "num_ctx": 4096.0, "num_predict": 120.0, "top_p": 0.4, "seed": 9.0, "stop": []any{}}, requests[1]["options"])
	assert.Equal(t, map[string]any{"temperature": 0.0, "num_ctx": 4096.0, "top_p": 0.0, "seed": 0.0, "stop": []any{}}, requests[2]["options"])
}

func TestOllamaMetadataPromptSettingsDoNotLeakAcrossConcurrentCalls(t *testing.T) {
	...
	model, err := newOllamaMetadataModel(server.URL, "model", 0, nil, 0.1, nil, OllamaMetadataConfig{Prompts: map[string]OllamaGenerationSettings{
		"first":  {Temperature: floatPointer(0.2)},
		"second": {Temperature: floatPointer(0.8)},
	}})
	require.NoError(t, err)
	var group sync.WaitGroup
	for filename, prompt := range map[string]string{"first": "one", "second": "two"} {
		group.Add(1)
		go func(filename, prompt string) {
			defer group.Done()
			_, callErr := model.Call(withOllamaPrompt(context.Background(), filename), prompt)
