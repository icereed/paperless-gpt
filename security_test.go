...
func TestBricoproHQCORSMiddlewareAllowsAnyOrigin(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.Use(bricoproHQCORSMiddleware())
	r.OPTIONS("/api/bricoprohq/v1/health", func(c *gin.Context) {
		c.Status(http.StatusOK)
	})
	r.GET("/api/bricoprohq/v1/health", func(c *gin.Context) {
		c.Status(http.StatusOK)
	})
	...
	for _, origin := range []string{
		...
		req := httptest.NewRequest(http.MethodOptions, "/api/bricoprohq/v1/health", nil)
		...
		getReq := httptest.NewRequest(http.MethodGet, "/api/bricoprohq/v1/health", nil)
		getReq.Header.Set("Origin", origin)
		getW := httptest.NewRecorder()
		r.ServeHTTP(getW, getReq)

		assert.Equal(t, http.StatusOK, getW.Code)
		assert.Equal(t, "*", getW.Header().Get("Access-Control-Allow-Origin"))
	}
}

func TestBricoproHQCORSMiddlewareIgnoresOtherPaths(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.Use(bricoproHQCORSMiddleware())
	r.OPTIONS("/api/documents", func(c *gin.Context) {
		c.Status(http.StatusOK)
	})
