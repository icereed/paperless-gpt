...
var envRegistry = []EnvVar{
	...
	{Name: "LLM_TEMPERATURE", Category: "LLM", Secret: false, Default: "0", Description: "(Ollama metadata only) Sampling temperature for title, tag, and metadata generation. When unset, the native-client baseline is `0` unless `OLLAMA_SETTINGS_FILE` provides a default. Must be a non-negative finite number; invalid values are ignored with a warning. Does not apply to Vision OCR or other LLM providers."},
	...
	{Name: "OLLAMA_CONTEXT_LENGTH", Category: "LLM", Secret: false, Default: "", Description: "(Ollama only) Positive integer that sets NumCtx (context window) for the Ollama runner. If unset or `0`, it does not override a settings-file default or the model default."},
	...
	{Name: "OLLAMA_SETTINGS_FILE", Category: "LLM", Secret: false, Default: "", Description: "(Ollama metadata only) Optional startup-only JSON file with strict defaults and per-prompt generation settings."},
