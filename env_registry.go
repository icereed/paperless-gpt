...
var envRegistry = []EnvVar{
	...
	{Name: "OPENROUTER_ENFORCE_ZDR", Category: "LLM", Secret: false, Default: "false", Description: "When `true` and `OPENAI_BASE_URL` points at openrouter.ai, adds `\"provider\": {\"data_collection\": \"deny\"}` to every LLM/VisionLLM request so OpenRouter only routes to upstream model providers offering a Zero Data Retention guarantee. Requests to any other endpoint are unaffected. Narrows the upstream provider pool for the configured model — check https://openrouter.ai/<model>/providers to confirm more than one ZDR-capable provider exists before enabling this with a single-provider model, or a 429/5xx with no fallback can result."},
