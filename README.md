...
| `TAG_BLACK_LIST`                    | A comma-separated list of tag names the LLM may never suggest. Blacklisted tags are withheld from the `<available_tags>` list in the prompt *and* stripped from the model's response, so they cannot be applied even with `CREATE_NEW_TAGS` enabled. Matching is case-insensitive. Tags a document already carries are never removed by this setting — it only suppresses new suggestions. Use it for paperless-gpt's own bookkeeping tags (`FAIL_TAG`, `AUTO_TAG_COMPLETE`, `PDF_OCR_COMPLETE_TAG`), which are otherwise offered to the model as if they were content tags. Example: `paperless-gpt-failed, paperless-gpt-auto-complete`. | No       |                            |
