...
func TestGetDocumentCountByTag(t *testing.T) {
	...
	ctx := context.Background()
	countAvblb, err := env.client.GetDocumentCountByTag(ctx, "avbl")
	require.NoError(t, err)
	assert.Equal(t, 5, countAvblb)

	countNavbl, err := env.client.GetDocumentCountByTag(ctx, "ntavbl")
	require.NoError(t, err)
	assert.Equal(t, 0, countNavbl)
}
